...
загружается рантаймом из `js/clarity.js`; домены разрешены в CSP всех
страниц). Учётная запись — у владельца; ключей/секретов в репозитории нет.

Дополнение 2026-08-13 (`c064543`, `05f0ab5` осн. репо): + IndexNow API
(`api.indexnow.org`, версия не пиннится) — вызов из workflow
`.github/workflows/indexnow.yml` при деплое; ключ — файл
`54435241f92a34641164ae1e98d2923c.txt` в корне сайта (по протоколу IndexNow
публичен, не секрет). + Bing Webmaster Tools (внешний сервис, аккаунт
владельца; в коде — только метатег верификации `msvalidate.01` в
`index.html`).
