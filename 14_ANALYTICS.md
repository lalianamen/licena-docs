...
`claude/registration-payment-report-metrics-h99xso` осн. репо, коммиты
`efbb582` + `2070b81`, `supabase/functions/daily-stats/index.ts`):
...
- **Из них оплатили (те же 7 дней):** когорта — из не-тестовых аккаунтов
  `auth.users`, созданных в эти 7 Pacific-дней, берутся те, у кого есть хотя
  бы одна строка `user_courses` с непустым `stripe_subscription_id`
  (учитываются и строки со статусом не `active` — оплата была). Считается по
  людям: один пользователь с двумя платными курсами — один оплативший.
  Уточнение владельца от того же 2026-08-15 («по оплате тоже за неделю, а не
  за все время») — первоначальная версия строки считала оплату за всё время.
  В письме явно сказано, что для последних дней число заведомо неполное:
  после регистрации идёт 3-дневный триал, часть когорты оплачивает уже за
  пределами окна.

Значения для блока возвращает `usersSection` (`{ html, payers, registered }`),
которой список дней окна передаётся аргументом `in7` — отдельных запросов к БД
блок не делает, и все три строки воронки описывают одно и то же окно, что и
7-дневная таблица. Fail-soft сохранён: при сбое секции
