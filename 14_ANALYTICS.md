...
· 2026-08-15 (точечная: блок «Воронка (KPI)»)
...
## Ежедневный отчёт (`daily-stats`, 461 строка на 2026-08-15, прочитан полностью)
...
## Дополнение 2026-08-15: блок «Воронка (KPI)» в daily-stats

По запросу владельца («процент людей, которые зашли на сайт и
зарегистрировались, и процент пользователей, которые зарегистрировались и
оплатили») строка «KPI: новые регистрации · конверсия» в ежедневном письме
заменена блоком «Воронка (KPI)» из трёх строк (ветка
`claude/registration-payment-report-metrics-h99xso` осн. репо, коммит
`efbb582`, `supabase/functions/daily-stats/index.ts`):

- **Вчера:** устройства с 2+ просмотрами за вчерашний Pacific-день →
  регистрации из `daily_signups` за тот же день → процент. Числа те же, что
  давала прежняя строка KPI.
- **За 7 дней:** те же 7 Pacific-дней, что перечисляет 7-дневная таблица
  письма (список дней `last7` вычисляется один раз и используется обоими).
  Знаменатель — устройства с 2+ просмотрами за всё окно (счётчик просмотров
  пересчитывается по окну, поэтому устройство, приходившее несколько дней,
  учитывается один раз, а не суммируется по дням); числитель — сумма
  `daily_signups` по этим дням.
- **Регистрация → оплата (за всё время):** число РАЗЛИЧНЫХ не-тестовых
  `user_id`, у которых есть хотя бы одна строка `user_courses` с
  непустым `stripe_subscription_id` (учитываются и строки со статусом не
  `active` — оплата была), делённое на число не-тестовых аккаунтов
  `auth.users`. Считается по людям: один пользователь с двумя платными
  курсами — один оплативший.

Значения для блока возвращает `usersSection` (`{ html, payers, registered }`)
— отдельных запросов к БД блок не делает. Fail-soft сохранён: при сбое секции
«Пользователи» письмо уходит, а строка оплаты печатает «нет данных — секция
«Пользователи» ниже не собралась». Новых секретов и SQL не требуется; деплой
ручной (владелец вставляет файл в Edge Function). Статус на 2026-08-15: код в
ветке основного репо, влит ли в `main` и задеплоен ли в Supabase — UNKNOWN
(за владельцем).
