...
- Блок «Воронка (KPI)» (2026-08-15, ветка
  `claude/registration-payment-report-metrics-h99xso`, коммиты `efbb582` +
  `2070b81`): визит→регистрация за вчера и за те же 7 дней, что в таблице
  письма (устройство считается один раз за окно), и «из них оплатили» —
  когорта тех же 7 дней (не-тестовые аккаунты, созданные в эти дни, у которых
  есть строка `user_courses` с `stripe_subscription_id`, включая неактивные).
