...
- AI-ассистент: Edge Function `assistant`, модель `claude-haiku-4-5` в `main` с `e266e4b` (PR #190, решение владельца 2026-09-11; до `a3d05d6` — `claude-sonnet-4-6`); функция передеплоена владельцем 2026-09-11 (сообщение владельца в сессии: «асистент обновлен»),
...
- Взято из header-комментариев файлов без построчной сверки с реализацией:
  роли модулей в таблице «Модули JS».
- Обновление 2026-08-05 (итерация «архитектура»): прочитаны полностью все
  SQL-файлы (`04_DATABASE.md`), `stripe-webhook/index.ts`,
  `stripe-portal/index.ts`; функции `ticket-*`, `daily-stats`, `assistant`
  прочитаны в объёме, указанном в `05_API.md` — утверждения этого документа о
  платежах, анти-шеринге и тикетах теперь подтверждены исходным кодом.
- Детализация: БД — `04_DATABASE.md`; API-контракты — `05_API.md`;
  функциональность — `13_UX.md`.
