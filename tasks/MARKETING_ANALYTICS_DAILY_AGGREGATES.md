...
**DONE** (запись 2026-09-11): код в `main` (`ae13124`), слой работает в production,
backfill и cron подтверждены. Незакрытым остаётся только то, что требует действий владельца
в Supabase (повторная проверка грантов после `revoke`, наблюдение за автозапуском cron).
Ревью ChatGPT из цикла убрано (ADR-002). Историческая шапка ниже сохранена.
