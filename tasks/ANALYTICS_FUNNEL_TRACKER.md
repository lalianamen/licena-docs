...
**DONE — код в production, серверные шаги выполнены владельцем 2026-09-11** (детали ниже)

Ранее: **CODE RELEASED — ждёт шагов владельца в Supabase** (2026-09-11, команда «да мерж, а я займус базой»):
...
- `daily-stats` с телом `{"week":"2026-09-07"}` (владелец, ~18:10 UTC): ответ
  ...
  (в т.ч. строка Bing) владелец проверяет в почте.
- Cron `bing-sync` создан владельцем (`cron.schedule` вернул jobid 17; расписание `30 14 * * 1`).

Все шаги Runbook выполнены 2026-09-11. Открытым остаётся только ручное: проверка текста письма,
удаление тестовой строки `app_events` (`meta.test = true`), ключевые события в GA4 (по желанию),
решение по возможному дублю более ранней Bing-синхронизации вне репозитория (UNKNOWN).
