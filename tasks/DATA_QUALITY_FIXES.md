...
Дополнение 3 (2026-09-14): после передеплоя владелец вызвал `marketing-aggregates` с
`{"days": 40}` — ответ `ok:true`, `days_written: 40` (2026-08-04 … 2026-09-12), `channel_rows: 116`,
`page_rows: 558`, `snapshot_day: 2026-09-13`, `rows_read: 2832`. Агрегаты восстановлены.
**Поправка к Дополнению 2:** `rows_read: 2832` за 68 дней означает, что дневной запуск (3 дня,
окно 31 день, ~1300 строк) в старом коде укладывался бы в лимит процессора; лимит объясняет отказ
40-дневного пересчёта, но НЕ подтверждает причину молчания ежедневной задачи с 2026-09-10 —
