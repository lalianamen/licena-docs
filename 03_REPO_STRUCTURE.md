...
Дополнение 2026-08-11 (`1e61960`): + `landing.css` (163 строки) — тёмная тема
navy+gold ТОЛЬКО для `index.html` (переопределение токенов после `styles.css`
+ стили новых продающих секций; auth-модал и саппорт пере-закреплены
светлыми). Парный JS: `js/landing-extras.js` (117 строк) — калькулятор цели и
живой вопрос C-10 на лендинге (данные — `js/samples/c10-exam.js`).
