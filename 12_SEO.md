...
Последняя сверка: 2026-08-05 (полная) · 2026-08-10 (точечная: HVAC-калькулятор) · 2026-08-13 (точечная: Bing WMT + IndexNow) · 2026-08-27 (точечная: NV practice-страницы) · 2026-09-17 (точечная: +6 practice-страниц, allowlist `EXAMS` в `js/seo.js`) · 2026-09-20 (точечная: лендинг — пикер, каталог по штатам) · 2026-09-21 (точечная: шапка лендинга — четыре штата) · 2026-09-23 (точечная: +3 practice-страницы la-mechanical-work на ветке, sitemap 167 → 170)
...
## Аддендум 2026-09-21 — SEO-выводы маркетингового анализа

Источник: `docs/marketing/analysis-2026-09.md` (осн. репо, ветка
`claude/question-bank-generation-analysis-y47sk7` @ `aec801a`). Измерено 2026-09-21
по `main` @ `8ea3685`. Изменений в `js/seo.js`, `sitemap.xml` и `robots.txt` не вносилось —
ниже зафиксировано текущее состояние, а не правки.

**Выключенные индексируемые активы.** 30 страниц калькуляторов (10 инструментов ×
EN/ES/RU) отдают `noindex, follow`, отсутствуют в `sitemap.xml` и не имеют ни одной
входящей внутренней ссылки — при том что canonical, hreflang и описания у них
проставлены. Аналогично `noindex` стоит на четырёх страницах роадмапа:
`roadmap.html`, `roadmap-az.html`, `roadmap-nv.html`, `roadmap-la.html`.
Проверка: `grep -rl noindex tools es/tools ru/tools | wc -l` → 30;
`grep -l noindex roadmap*.html` → 4.

**Дополнение 2026-09-23 (на ветке, не в проде).** Добавлены `/practice/la-mechanical-work/`
+ `/es/…` + `/ru/…` — `sitemap.xml` 167 → 170 URL, `check-offer.js` 84 платные страницы,
practice-каталогов 34 экзамена × 3 языка. Slug `la-mechanical-work` добавлен в allowlist
`EXAMS` в `js/seo.js` тем же коммитом (правило от 2026-09-17), `seo.js?v=22` на трёх новых
страницах. Гидов под Луизиану по-прежнему 0: перекос ниже остаётся, LA теперь 0 гидов при
3 банках на ветке (2 в проде).

**Перекос информационного слоя относительно коммерческого.** 20 гидов (60 страниц)
против 27 платных банков: CA — 13 гидов на 15 банков (нет под A, C-33, C-54),
AZ — 3 гида общего характера ни под одну классификацию при 5 банках,
**NV — 0 гидов при 5 банках**, **LA — 0 при 2**. Внешняя разведка показала, что
именно в AZ/NV/LA на первой странице выдачи нет конкурента нашего класса, то есть
информационный слой отсутствует ровно там, где вход дешевле всего.

**Язык как ранжирующий фактор — подтверждение первоисточником.** Августовский
GSC-срез показывал, что испанские страницы ранжируются структурно выше английских
(ES C-27 — позиция 1, ES-главная ≈ 7.6 против 40–80 у английских головных запросов).
Первоисточники объясняют почему и где это оправданно контентом:
CSLB предлагает испанские версии 10 экзаменов (совпадают 8 наших CA-банков),
NSCB с 10.12.2025 — испанские экзамены для 13 классификаций Невады + CMS
(совпадают `nv-cms`, `nv-b2`, `nv-c2`). Для этих страниц утверждение «экзамен
официально сдаётся по-испански» проверяемо и на сайте пока не используется.
Для остальных — использовать нельзя.

**Verified** — все числа получены командами над рабочей копией `main` @ `8ea3685`
21.09.2026 (команды приведены выше); внешние факты — по URL первоисточников,
перечисленным в разделах 5 и 6 документа анализа. Данные о позициях и трафике
после 05.08.2026 — **UNKNOWN** (нет доступа к Supabase из сессии).

### Исправление к аддендуму 2026-09-21

Утверждение выше о «30 страницах калькуляторов как выключенном индексируемом активе»
**ошибочно и отменяется**. Калькуляторы закрыты course-gate по решению владельца
от 2026-08-10: плеер курса ставит `lp:tool_gate` при клике по ссылке «Tools»
(`js/app-course.js:887`), а страница без свежей метки (окно 15 минут) выполняет
`location.replace("/")` — `js/trade-calcs.js:24` и `js/hvac-calc.js:25`. `noindex`
на этих 30 страницах согласован с гейтом, а не забыт. Открытие их для поиска —
продуктовое решение (отдать калькуляторы анонимным посетителям), а не SEO-правка;
до снятия гейта индексация бессмысленна: краулер получит редирект на главную.

Утверждение о четырёх страницах роадмапа остаётся, но уточняется: гейта у них нет,
анонимный посетитель работает с ними нормально. Однако `<main id="rmRoot">` пуст —
весь контент рисует JS, и в `<head>` нет ни `description`, ни `canonical`, ни
`hreflang`. Снятие `noindex` без статического вводного блока и мета-тегов даст
тонкий контент.

**Verified** — гейт прочитан в коде 2026-09-21; поведение воспроизведено в Chromium:
прямой переход на `/tools/concrete-calculator/` заканчивается на `https://licena.us/`
(в тесте — `http://127.0.0.1:8901/`), заголовок и DOM принадлежат лендингу.
