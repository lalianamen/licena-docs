...
Последняя сверка: 2026-08-05 · дополнение 2026-08-10 (см. примечание после таблицы) · 2026-08-30 (точечная: маркетинговые агрегаты) · 2026-09-09 (точечная: промо-видео)
...
| Edge Functions | `supabase/functions/*` | 05 (контракты), 06 (все 9 функций `main` прочитаны полностью, включая `marketing-aggregates`) | документировано | дашборд-настройки (Verify JWT, секреты) — UNKNOWN |
...
| Маркетинговые агрегаты | `supabase/sql/marketing-daily-aggregates.sql`, `cron-marketing-aggregates.sql`, `functions/marketing-aggregates/{core.ts,index.ts}`, `scripts/test-marketing-core.mjs`, `scripts/check-channel-parity.js` (`main` @ `ae13124`) | 14 (слой и отличия от письма), 04 (4 таблицы, RPC, cron), 06 (устройство функции), 08 (гранты/авторизация), 20 (проверки) | документировано | первый автоматический запуск по расписанию — UNKNOWN; гранты после `revoke` не перепроверены |
...
| SMM | `docs/smm/*` | 03 (перечень; `docs/smm/video/` — промо-видео со звуком, состав файлов), 26 (строки 2026-09-09: сцены и источник каждой строки видео; озвучка/музыка), 28 (механика очередей, где лежит видео) | частично | → `23_SMM_STATE.md`; промо-видео живёт в ветке `claude/lasena-video-creation-bgxgc9`, не в `main` |
