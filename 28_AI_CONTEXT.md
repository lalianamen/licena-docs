...
план этапов 1–4 закрыт; открыты Page-токен Meta до 2026-11-11 и деплой `daily-stats` (UNKNOWN); TikTok — вручную; договорённости: одна таблица
...
## Дополнение 2026-09-13 — вход по ссылке, каналы и cron

- **Просроченная ссылка входа теперь видна пользователю.** Supabase отдаёт
  ошибку в hash (`#error_code=otp_expired`); до 2026-09-13 её никто не читал и
  экран был пустой. Панель `data-panel="link-expired"` в `index.html`,
  логика — в `js/app.js`, повторная отправка письма — `supa.auth.resend`.
  **Ловушка на будущее**: `createClient` в `supabase-js` очищает hash ДО того,
  как выполнится `js/app.js`. Любой код, читающий hash после создания клиента,
  работает через раз. Метка перехватывается в `js/supabase-client.js` в
  `window.LP_LINK_ERROR` строго перед `createClient` — не переставлять.
- **Канал `youtube` добавлен в таксономию.** Правка канала = ТРИ места, а не
  два: `daily-stats/index.ts`, `marketing-aggregates/core.ts` (парити стережёт
  `scripts/check-channel-parity.js`) **и** CHECK-ограничение
  `marketing_channel_daily.channel` в базе. Порядок: сначала ALTER
  (`supabase/sql/marketing-channels-youtube.sql`), потом редеплой функций —
  иначе запись нового канала валит всю транзакцию замены за день.
- **У `gsc-sync` не было расписания.** Функция задеплоена с 2026-09-11, но
  cron-файла для неё никогда не существовало — снапшоты появлялись только при
  ручном вызове. Расписание: `supabase/sql/cron-gsc-sync.sql` (14:40 UTC, за 20
  минут до письма daily-stats). Владелец должен подставить два плейсхолдера и
  прогнать один раз.
- **Разметка ссылок для соцсетей** — единый формат в
  `docs/marketing/utm-links.md` (`utm_*` для GA4 + `src=<префикс>-<дата>-<тема>`
  для собственной аналитики; `src` выигрывает у `utm_source` намеренно).
- **Диагностика аналитики** — `supabase/sql/diagnose-marketing.sql` (read-only):
  состояние cron, свежесть агрегатов, сырые referrer-хосты и `src`-метки.
  Живого состояния cron в репозитории нет — почему `marketing_daily_metrics`
  не обновляется ежедневно, до прогона этих запросов **UNKNOWN**.
