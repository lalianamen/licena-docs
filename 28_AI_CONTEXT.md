...
Последняя сверка: 2026-08-05 (полная) · 2026-08-19 (точечная: Test Bank v2 на все банки)
...
- (дополнение 2026-08-19, `11a0f00`→`73149c3`→`a73d87f`→`4dba4eb`) **Test
  Bank v2 — стандартный UI плеера для ВСЕХ банков.** Дизайн-слой:
  `body.tb-v2` (ставится безусловно в начале `js/app-course.js`) +
  `css/test-bank-v2.css`, целиком скоупленный под `.tb-v2` и построенный на
  токенах лендинга из `styles.css`. Пилот на `asbestos` утверждён владельцем
  (desktop и mobile отдельно), затем гейт `TB_V2`-whitelist удалён.
  Плавающий Assistant-пузырь скрыт на тест-экранах; вместо него кнопка
  «? Help» → трёхъязычный шит `js/help-feedback.js`, передающий контекст
  текущего вопроса (`window.LICENA_qContext` → `LICENA_helpPrefix` в
  `js/support.js`) в ту же панель ассистента. Все v2-ветки читают данные
  текущего курса (totals, `EXAM_FORMATS`/`EXAM_MINUTES`, блок-мета,
  `COURSE_REF`); контент банков и логика скоринга/прогресса/экзамена не
  менялись. Дизайн-оценочная страница `preview.html` (реальный плеер на
  бесплатных банках без входа, `js/preview-stub.js`) — регенерируется из
  `course.html` при каждом его изменении.
...
- `CLAUDE.md` основного репо говорит о «12 paid banks» на `content-banks-src`
  ...
  с 2026-08-16 на ветке `claude/az-b-general-contractor-9zj4e9` готов и
  подключён 15-й платный курс `az-b` (500×3, все шесть платных списков,
  check-paid-sync «15 courses»), в `main` не влит.
