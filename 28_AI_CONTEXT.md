...
- (дополнение 2026-08-10, итог `abd8804`) HVAC-калькулятор: страницы
  ...
  любому банку одной записью; с `4234bff` — 8 калькуляторов
  (`TOOL_DEFS`: hvac, electrical, plumbing, roofing, concrete, painting,
  solar, fire, pricing) на 16 курсах, 24 страницы `/tools/**` ×3 языка,
  общая логика `js/trade-calcs.js`. ВАЖНО: страницы `/tools/` не подключают
