...
- (дополнение 2026-08-11, `1e61960`) Лендинг `index.html` — тёмная navy+gold
  продающая страница (утверждённый владельцем синтез Lovable-образцов из
  `lalianamen/licena-insights`): тема — `css/landing.css` (грузится ТОЛЬКО
  index.html; переопределяет токены после `styles.css`, auth-модал остаётся
  светлым); интерактив — `js/landing-extras.js` (калькулятор цели на числах
  самого посетителя + живой вопрос из `js/samples/c10-exam.js` с EN-тумблером
  и полным объяснением); `js/i18n.js` v44. Прежние хуки (`#navLogin`,
  auth-модал, `data-seo-*`, `testiSection`, support) сохранены; `js/seo.js`,
  sitemap, practice/guides не менялись. Переводы вердиктов/CTA и шаблонов
  окупаемости JS читает из скрытых `data-t`-спанов (`#lq-right` и т. п.) —
  не переименовывать эти id.
