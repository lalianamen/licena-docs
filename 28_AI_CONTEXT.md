...
Последняя сверка: 2026-08-05 (полная) · 2026-08-11 (точечная: редизайн лендинга) · 2026-08-24 (точечная: Невада) · 2026-08-25 (точечная: мерж NV; nv-cms и nv-b готовы полностью; nv-b2 Run 1 — blueprint)
...
видна, банк пуст). Порядок линейки изменён 2026-08-25: `nv-c2`/`nv-c21`/`nv-c1d` заблокированы доступом к NEC 2017/UMC 2018/UPC 2018 (up.codes гейтит NFPA/IAPMO-титулы логином; детали — `docs/content/nv-b-ledger.md`, «Source-access verification»). **`nv-b` (B General Building) ГОТОВ полностью** (Runs 1–7, 2026-08-25): на `content-banks-src` — 500 вопросов × EN/RU/ES (ключи A125 B125 C125 D125, финальный коммит `aefecbd`; каждое IBC-число утверждалось дословно в двух адопциях NV+WY; 10% выборка ключей — 50/50; полнобанковый числовой паритет 1–500 по всем полям и языкам; блюпринт `93548c7`, леджер до `790e486`); wiring на ветке осн. репо `claude/question-bank-generation-analysis-y47sk7` (`77d909b`): EXAM_FORMATS `"nv-b" {count:80, passPct:70}` + EXAM_MINUTES 180, карточка в `nv-nscb`, COURSE_REF, запись bank-updates — ЖДЁТ мержа владельцем; CSV регенерирован с nv-b: 33,500 строк / 22 платных банка (~35 MB) — ждёт импорта в Supabase. Дальше линейка СТОИТ до получения NEC 2017 (+NFPA 72), UMC 2018, UPC 2018 или логина up.codes. **Следующий банк — `nv-b2` (B-2 Residential & Small Commercial), Run 1 выполнен 2026-08-25** по команде владельца «начинай следующий платный банк»: blueprint + леджер-скелет на ветке осн. репо (`18d38ef`, `docs/content/nv-b2-blueprint.md` / `nv-b2-ledger.md`); экзамен 80 в. / 3 ч / 70%, open book IBC 2018 + IRC 2018 + ACI 318; double-adoption пары: IRC 2018 NV+ID (проверены 2026-08-25, резерв — OK), IBC 2018 NV+WY (конвейер nv-b); IRC — источник по умолчанию (каждый IRC-факт нов против nv-b по построению), IBC-факты — только после grep-гейта по леджеру nv-b. Линейка nv-c2/c21/c1d при разблокировке источников имеет приоритет. Примечание к цифрам
