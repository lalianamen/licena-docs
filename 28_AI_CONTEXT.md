...
Последняя сверка: 2026-08-05 (полная) · 2026-08-11 (точечная: редизайн лендинга) · 2026-08-24 (точечная: Невада)
...
Дополнение 2026-08-24: на ветке `claude/question-bank-generation-analysis-y47sk7`
основного репо подготовлено открытие Невады (третий штат кабинета) — NV в
`STATES` + `js/catalog/nv.js` с 4 федеральными бесплатными банками (те же
course id, что в CA/AZ) — ждёт мержа владельцем; там же анализ выполнимости
генерации NV-банков `docs/content/nv-feasibility.md` (линейка nv-cms → nv-c2 →
nv-c21 → nv-c1d → nv-b по бюллетеням PSI от 25.03.2026). Примечание к цифрам
...
внесены (см. пробел покрытия в `26_CHANGELOG.md`). Там же (та же ветка,
`435946f`) подготовлены practice-страницы четырёх AZ trade-банков: 12 страниц
+ `js/samples/az-*.js` + 4 slug в `EXAMS` + sitemap 137 URL — ждёт мержа.
